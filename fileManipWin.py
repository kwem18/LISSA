...
def cut(original,prefix,fSize,debug=0):
	...
	bytes = [" -v",str(fSize),"b"]
	command = ["7za a "]+[prefix,' ',original]+bytes
	sysCall(command)
...
def comb(prefix,fileName,rmv=0):
	...
	command = "7za x "+prefix+' -tsplit' #This is going to need to be reworked so that the input file can be combined and unzipped in one step for the user
	sysCall(command)
	
	command = "7za e "+prefix
